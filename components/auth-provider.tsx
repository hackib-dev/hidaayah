...
import { fetchReflectProfile } from '@/app/(app)/dashboard/profile/queries';
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [reflectProfileFetched, setReflectProfileFetched] = useState(false);
  ...
  // Only fires once after a fresh login (user wasn't in storage on mount).
  // reflectProfileFetched prevents infinite re-runs when the profile returns null.
  useEffect(() => {
    if (user && !reflectProfileFetched && !reflectProfileLoading) loadReflectProfile();
  }, [user, reflectProfileFetched, reflectProfileLoading, loadReflectProfile]);
