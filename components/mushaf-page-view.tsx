...
import { ChevronLeft, ChevronRight, Loader2, Check, Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, Settings2, X } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { fetchVersesByPage, fetchVerseAudioFiles, fetchVerseReciters } from '@/app/(app)/dashboard/quran/queries';
...
export function MushafPageView({ startPage, chapterName, onPageChange }: MushafPageViewProps) {
  ...
  useEffect(() => {
    ...
    const surahNums = [...new Set(
      verses.map((v) => v.chapter_id || parseInt(v.verse_key.split(':')[0], 10))
    )].filter(Boolean);
    Promise.all(
      surahNums.map((s) => fetchVerseAudioFiles(selectedReciterId, s).catch(() => []))
    ).then((all) => {
  ...
  useEffect(() => {
    ...
    if (!file) { audio.pause(); return; }
    if (audio.src !== file.url) {
      audio.src = file.url;
      audio.load();
      if (isPlaying) {
        // Wait for canplay before playing after a src change
        const onCanPlay = () => { audio.play().catch(() => setIsPlaying(false)); audio.removeEventListener('canplay', onCanPlay); };
  ...
  const skipPrev = () => {
    const idx = audioFiles.findIndex((a) => a.verse_key === currentVerseKey);
    if (idx > 0) { const k = audioFiles[idx - 1].verse_key; setCurrentVerseKey(k); setActiveVerseKey(k); }
  };

  const skipNext = () => {
    const idx = audioFiles.findIndex((a) => a.verse_key === currentVerseKey);
    if (idx >= 0 && idx < audioFiles.length - 1) { const k = audioFiles[idx + 1].verse_key; setCurrentVerseKey(k); setActiveVerseKey(k); }
  };
  ...
  const SidebarContent = () => (
    <div className="flex flex-col gap-3">
      ...
      <div className="rounded-2xl border border-border bg-card p-3 space-y-3">
        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Audio</p>
        ...
        <div className="flex items-center justify-between">
          <button onClick={skipPrev} disabled={currentIdx <= 0} className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground transition-colors disabled:opacity-30">
          ...
          <button onClick={skipNext} disabled={currentIdx >= audioFiles.length - 1} className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground transition-colors disabled:opacity-30">
            <SkipForward className="w-4 h-4" />
          </button>
          <button onClick={() => setIsMuted((m) => !m)} className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground transition-colors">
        ...
        {reciters.length > 0 && (
          <select
            ...
            onChange={(e) => { setSelectedReciterId(Number(e.target.value)); setIsPlaying(false); setCurrentVerseKey(null); setActiveVerseKey(null); }}
      ...
      <div className="rounded-2xl border border-border bg-card p-3 space-y-2">
        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Script</p>
        <div className="flex flex-col gap-1.5">
          {FONT_OPTIONS.map((opt) => (
            <button key={opt.id} onClick={() => setFont(opt.id)}
              className={cn('flex items-center gap-2 px-2.5 py-2 rounded-xl border text-left transition-colors', font === opt.id ? 'border-primary bg-primary/5 text-foreground' : 'border-border text-muted-foreground hover:text-foreground hover:border-primary/30')}
            >
              <span className="text-base leading-none shrink-0" style={{ fontFamily: opt.id === 'indopak' ? "'IndoPakNastaleeq', serif" : "'UthmanicHafs', serif" }}>{opt.sampleText}</span>
        ...
        <div className="flex flex-col gap-1.5">
          {THEME_OPTIONS.map((t) => (
            <button key={t.id} onClick={() => setTheme(t.id)}
              className={cn('flex items-center gap-2 px-2.5 py-2 rounded-xl border text-xs font-semibold transition-colors', theme === t.id ? 'border-primary' : 'border-transparent')}
        ...
        <div className="flex flex-col gap-1.5">
          {(['sm', 'md', 'lg'] as const).map((s) => (
            <button key={s} onClick={() => setFontSize(s)}
              className={cn('px-2.5 py-1.5 rounded-xl border text-xs font-semibold text-left transition-colors', fontSize === s ? 'border-primary bg-primary/5 text-foreground' : 'border-border text-muted-foreground hover:text-foreground')}
            >
      ...
      <div className="flex flex-col items-center flex-1 min-w-0">
        ...
        <div className="w-full flex items-center justify-between px-1 py-1.5 mb-2">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground font-medium tabular-nums">صفحة {page}</span>
            {/* Mobile-only toggle button */}
            <button
              onClick={() => setSidebarOpen((o) => !o)}
              className={cn(
                'lg:hidden p-1.5 rounded-lg transition-colors',
                sidebarOpen ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-secondary'
              )}
            >
        ...
        <div
          ...
          style={{ background: themeConfig.bg, border: `1px solid ${themeConfig.border}`, minHeight: 480 }}
        >
          {loading ? (
            <div className="flex items-center justify-center h-64">
              <Loader2 className="w-6 h-6 animate-spin" style={{ color: themeConfig.text, opacity: 0.4 }} />
            </div>
          ) : (
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {verses.length === 0 && (
                  <p className="text-center py-12 text-sm" style={{ color: themeConfig.text, opacity: 0.5 }}>
