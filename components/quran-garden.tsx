...
function GardenScene({ state, vitality }: { state: GardenState; vitality: number }) {
  ...
  const skyColors =
    stage === 4 ? ['#fef3c7', '#d1fae5'] :
    stage === 3 ? ['#e0f2fe', '#dcfce7'] :
    stage === 2 ? ['#f0fdf4', '#ecfdf5'] :
                  ['#f8fafc', '#f0fdf4'];
  ...
  return (
    <svg
      ...
      style={{ filter: dim ? 'saturate(0.5) brightness(0.85)' : 'none', transition: 'filter 1.5s ease' }}
    >
      <defs>
        ...
        <linearGradient id="ground" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={stage >= 3 ? '#4ade80' : stage >= 2 ? '#86efac' : '#bbf7d0'} />
      ...
      {stage >= 2 && (
        <motion.circle
          cx="680" cy="60" r={stage >= 4 ? 38 : 28}
          fill={stage >= 4 ? '#fde68a' : '#fef9c3'}
          animate={{ r: [stage >= 4 ? 38 : 28, stage >= 4 ? 42 : 31, stage >= 4 ? 38 : 28] }}
          transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
        />
      )}
      {u.includes('golden_light') && (
        <ellipse cx="680" cy="60" rx="80" ry="80" fill="url(#glow)" />
      )}

      {/* Distant hills */}
      {stage >= 2 && (
        <>
          <ellipse cx="150" cy="280" rx="200" ry="80" fill={stage >= 3 ? '#86efac' : '#bbf7d0'} opacity="0.5" />
          <ellipse cx="650" cy="290" rx="180" ry="70" fill={stage >= 3 ? '#86efac' : '#bbf7d0'} opacity="0.4" />
        </>
      )}
      ...
      {/* Rich grass blades */}
      {u.includes('rich_grass') && Array.from({ length: 30 }).map((_, i) => (
        <motion.line
          key={i}
          x1={20 + i * 26} y1="300"
          x2={20 + i * 26 + (i % 3 === 0 ? 4 : -4)} y2={285 - (i % 4) * 3}
          stroke="#22c55e" strokeWidth="2.5" strokeLinecap="round"
          animate={{ x2: [20 + i * 26 + (i % 3 === 0 ? 4 : -4), 20 + i * 26 + (i % 3 === 0 ? -3 : 3), 20 + i * 26 + (i % 3 === 0 ? 4 : -4)] }}
          transition={{ duration: 2 + (i % 3) * 0.5, repeat: Infinity, ease: 'easeInOut', delay: i * 0.07 }}
        />
      ))}
      ...
      {u.includes('pathway') && (
        <>
          ...
          {Array.from({ length: 6 }).map((_, i) => (
            <ellipse key={i} cx="400" cy={370 - i * 18} rx={55 - i * 4} ry="8"
              fill={u.includes('glow_path') ? '#fef08a' : '#fde68a'} opacity={u.includes('glow_path') ? 0.5 : 0.25} />
          ))}
        </>
      )}
      ...
      {u.includes('stream') && (
        <motion.path
          ...
          fill="none" stroke="#7dd3fc" strokeWidth="8" strokeLinecap="round" opacity="0.7"
          animate={{ d: [
      ...
      {u.includes('waterfall') && (
        <>
          ...
          <motion.rect x="60" y="240" width="18" height="60" fill="#bae6fd" opacity="0.4" rx="4"
            animate={{ y: [240, 244, 240] }} transition={{ duration: 1.2, repeat: Infinity }} />
      ...
      {u.includes('small_flowers') && [
        { x: 80, y: 295, c: '#f9a8d4' }, { x: 160, y: 292, c: '#fde68a' },
        { x: 580, y: 294, c: '#a5f3fc' }, { x: 660, y: 291, c: '#f9a8d4' },
      ...
      {u.includes('rare_flowers') && [
        { x: 300, y: 290, c: '#c084fc' }, { x: 500, y: 288, c: '#fb923c' },
      ...
      {u.includes('first_tree') && (
        <motion.g animate={{ rotate: [-1, 1, -1] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }} style={{ transformOrigin: '200px 300px' }}>
      ...
      {u.includes('wisdom_tree') && (
        <motion.g animate={{ rotate: [-1, 1, -1] }} transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut' }} style={{ transformOrigin: '600px 300px' }}>
      ...
      {u.includes('fruit_tree') && (
        <motion.g animate={{ rotate: [-0.8, 0.8, -0.8] }} transition={{ duration: 4.5, repeat: Infinity, ease: 'easeInOut' }} style={{ transformOrigin: '400px 300px' }}>
          ...
          {/* Fruits */}
          {[{ x: 390, y: 215 }, { x: 410, y: 220 }, { x: 398, y: 235 }, { x: 415, y: 238 }, { x: 382, y: 230 }].map((p, i) => (
            <motion.circle key={i} cx={p.x} cy={p.y} r="6" fill="#ef4444"
      ...
      {u.includes('reflection_pool') && (
        <>
          <motion.ellipse cx="400" cy="360" rx="70" ry="22" fill="#7dd3fc" opacity="0.5"
            animate={{ rx: [70, 74, 70] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }} />
          <ellipse cx="400" cy="360" rx="70" ry="22" fill="none" stroke="#bae6fd" strokeWidth="1.5" opacity="0.6" />
          {/* Reflection shimmer */}
          {[0, 1, 2].map((i) => (
            <motion.line key={i}
              x1={360 + i * 20} y1="358" x2={365 + i * 20} y2="362"
      ...
      {u.includes('circle_grove') && [
        { x: 100, y: 270 }, { x: 130, y: 265 }, { x: 115, y: 258 }
      ].map((t, i) => (
        <motion.g key={i} animate={{ rotate: [-1, 1, -1] }} transition={{ duration: 3 + i, repeat: Infinity }} style={{ transformOrigin: `${t.x}px 300px` }}>
      ...
      {/* Birds */}
      {u.includes('birds') && [
        { x: 300, y: 80 }, { x: 340, y: 70 }, { x: 370, y: 85 }
      ].map((b, i) => (
        <motion.g key={i}
      ...
      {stage === 1 && state.totalXP < 10 && (
        <>
          <text x="400" y="185" textAnchor="middle" fill="#9ca3af" fontSize="13" fontFamily="Georgia, serif" opacity="0.7">
            Begin your Quran journey...
          </text>
          <text x="400" y="205" textAnchor="middle" fill="#9ca3af" fontSize="11" fontFamily="Georgia, serif" opacity="0.5">
...
export function QuranGarden({ compact = false }: QuranGardenProps) {
  ...
  const stageLabels = ['Bare Beginning', 'Early Growth', 'Flourishing Garden', 'Spiritual Sanctuary'];
      ...
      <div className="rounded-2xl border border-border bg-card overflow-hidden">
        <div className="relative h-32">
          ...
          <div className="absolute bottom-2 left-3 right-3 flex items-end justify-between">
            <div>
              <p className={cn('text-xs font-bold', stageColors[stage - 1])}>{stageLabels[stage - 1]}</p>
              <p className="text-[10px] text-muted-foreground">Level {level} · {totalXP} XP</p>
            </div>
    ...
    <div className="space-y-4">
      ...
      <div className="relative rounded-2xl border border-border overflow-hidden shadow-sm bg-card">
        ...
        <div className="px-4 py-2 border-t border-border flex items-center gap-2">
          ...
          <div className="flex-1 h-1.5 rounded-full bg-secondary overflow-hidden">
            <motion.div
              className={cn('h-full rounded-full', vitality > 70 ? 'bg-sky-400' : vitality > 40 ? 'bg-amber-400' : 'bg-rose-400')}
      ...
      <div className="rounded-2xl border border-border bg-card p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{current} / {needed} XP to next level</p>
          </div>
          <div className="text-right">
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span className="flex items-center gap-1"><Star className="w-3 h-3 text-amber-500" />{weeklyXP} XP this week</span>
      ...
      {state.unlockedElements.length > 0 && (
        <div className="rounded-2xl border border-border bg-card p-4 space-y-2">
          <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Garden Elements</p>
          <div className="flex flex-wrap gap-2">
            {state.unlockedElements.map((el) => (
              <span key={el} className="px-2.5 py-1 rounded-xl bg-emerald-600/10 text-emerald-700 dark:text-emerald-400 text-xs font-semibold capitalize">
                {el.replace(/_/g, ' ')}
              </span>
            ))}
          </div>
      ...
      {/* Action counts */}
      <div className="rounded-2xl border border-border bg-card p-4 space-y-2">
        <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Your Journey</p>
        <div className="grid grid-cols-2 gap-2">
          {([
            ...
            <div key={label} className="flex items-center justify-between p-2.5 rounded-xl bg-secondary/50">
